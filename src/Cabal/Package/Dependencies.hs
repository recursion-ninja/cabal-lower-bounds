...
withPinnedLowerBound (Dependency p v s) = Dependency p v' s where v' = withinVersion $ versionLowerBound v
...
versionLowerBound vRange =
    ...
    in  if isAnyVersion vRange then defaultRange else extractRange vRange
