...
module GHC.Distribution.Table.Load
...
moduleName :: String
moduleName = $(qLocation >>= (pure . LitE . StringL) . loc_module)
...
parseTabularStructure =
    let compileTimeError = errorMessage "Empty file...!?" in fromMaybe compileTimeError . uncons . lines
...
parseHeader =
    let groupByFour i = \case
        ...
        suffixing rank entity =
            ...
            in  if expect == actual then Just prefix else Nothing
        ...
        groupErrorMessage i j = parseErrorMessage $ fold [errorPrefix i, ", found only (", show j, "/4)"]
        ranksErrorMessage i =
            parseErrorMessage $ fold [errorPrefix i, ", suffix(es) do not match version rank(s)"]
        namesMessage i = parseErrorMessage $ fold [errorPrefix i, ", all four names are not the same"]
...
parseRowCSV
    ...
    -> Char  -- ^ Fields separated by
    -> String
    -> Either Word [String]
parseRowCSV enc sep =
    let go :: Word -> String -> Either Word [String]
        go i input = case cell $ trim input of
            Nothing             -> Left i
            Just (extra, value) -> case trim extra of
        ...
        cell = let start str = (str, mempty) in quoted <=< within <=< quoted . start

        quoted :: (String, String) -> Maybe (String, String)
        quoted (str, v) = case str of
            c : d | c == enc -> Just (d, v)
            _                -> Nothing

        within :: (String, String) -> Maybe (String, String)
        within (str, _) = pure . swap $ break (== enc) str
    in  go 1
